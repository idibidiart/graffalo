...
Microservices, and particularly Entity-Mapped Microservices, imply the use of distributed transactions that access entities asynchronously, across separate databases of different types (relational, hierarchical, unconstrained graph, etc) including performig causally-dependent read and write operations, and from multiple clients, concurrently. 
...
When updating multiple entities via a distributed transaction in the presence of concurrency, we can use a transaction manager service that manages a two-phase commit process across the databases that have the entities in question so we may guarantee atomicity, but the transaction manager would become the bottleneck and single point of failure and is therefore not a good fit if we require both maximum availability and maimum concurrency. 

One approach to is to eliminate distributed transactions by lumping services into domain model aggregates (all related entities can be accessed via one API, such that orchestration and aggregation happen within one database transaction) which alloews us to break out of the pure entity-mapped API model and be able to use the database' 'strict serializability' isolation level (aka linearizability) to assure atomicity of the transaction and consistency of our data at any given time, i.e. strong consistency. However, this removes the granular access needed to easily define and change our GraphQL schema and application data model on demand (without changing the underlying REST APIs) -- To solve that problem, see The Great API Masher (coming soon.)
