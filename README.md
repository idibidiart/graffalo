...
Also, instead of fetching deeply nested structures all at once we can use GraphQL tree-based pagination to fetch in stages as the user epxlores the nested data, which is to say we paginate the edges at each node. We can also avoid ending up with N+1 queries for nested 1-to-many relations by using libraries like graphql-resolve-batch to batch the edges at each node into one resolver invocation (with a special resolver function that works with arrays of values as source) 
