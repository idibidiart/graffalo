...
When updating multiple entities via a distributed transaction in the presence of concurrency, we can use two-phase commit to achieve atomicity, but that works based on locking and is therefore not a good fit if we require both maximum availability and maimum concurrency. 
