...
Moreover, if the API output results in many repeated entries (e.g. getting movie times for the day across many theaters) the JSON to be parsed will be larger than it needs to be if the API output was de-duped (with type name and id's remaining in output, but not the content of each item) Also, instead of fetching deeply nested structures all at once we can split the query to fetch in stages as the user epxlores that nested data and cache previously fetched and paginate at every level. 
