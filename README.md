...
The Apollo GraphQL client, which we've chosen to use in this architecture, persists query results in a local cache (for the purpose of caching and/or composing app state in the client for Optimistic and Offline-First UI implementations) The use of forceFetch (bypass cache) in Apollo client can work nicely with Feathers realtime mutation events in that we may set forceFetch each time landing on a given route in the client.

After loading the data, instead of having to fetch app state from the server continuously to keep client state in sync, which does not scale, Feathers realtime events save the day by telling us only when there has been a change to the data. This means that each route on the client has to listen to those events that relate to it so it can update its data, and when the route comes up again we would fetch again from the server. This way we don't deal with invalidation of cached dynamic data, which is a recipe for bugs. The counter argument is to deal with the complexity of invalidation cache of dynamic data in return for less fetching form the server, but the truth is our data is already shaped to fit exactly what we need (no over fetching happening since we're using GraphQL queries that are specific to each part of the UI) and cache invalidation, done correctly, would cause more requests to the server. 

We leave it to the client to decide exactly what part of that area of app state it wants to refetch. So if you have e.g. a grid of 100K rows and UI is displaying rows 20-60, we won't care if rows 500-2500 have changed, so we don't need to push that data to the client. But we do have to tell the client which rows changed. 
