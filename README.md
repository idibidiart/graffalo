...
If we must have CPU-bound processes that maintain state in memory they should have their own crash recovery, using state machine replication for restoring state after a crash or clear-any-incomplete-output-and-restart type of process. Transient state like taking a user's order is held in the UI component, e.g. the order form, until it's committed via the service layer. 

## Distributed Transactions

A distributed transaction is one that touches many systems. For example, processing payment for food delivery with Visa or Stripe et al and then updating placing the order is a distributed transaction. What if the server crashes after payment is validated but before placing the order with the restaurant? We have nothing to rollback in the database, since the mutation in this case happened in an external system. 

Since we coordinate all transactions from the GraphQL mutation resolvers, we would have a table in our database for maintaining the order state and have a background process that looks at any stale orders and checks with the payment system to see if the payment was successful, in which case it would continue with placing the order. Else, it would push an event to the user to inform the of payment failure.   
...
In this context, a Feathers mutation event must be utilized as update 'signal' rather than update 'content.' We should not send any data. We should only sebd the topic name to indicate a certain area of app state has changed. Then we leave it to the client to decide exactly what part of that area of app state it wants to refetch. We must refetch the queries relevant to the given UI route every time we land on the route, when we receive a mutation signal that we've subscrived to for that route, and also after loss of network connectivity.

The current rendered state of the component that is interested in the given mutation signal can be used for query params such as the index of the last fetched item in a social status feed, so the status feed component, upon receiving a mutation signal from the server that is relevant to it, can get the latest items (from last index, which is passsed in as query param) We may generalize this pattern to UI components with arbitrary data structures. We do this because there is no way for the server to know what was the last state of the status feed on the client without the server becoming stateful and losing horizontal scalability or reliability.
