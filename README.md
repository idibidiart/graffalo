...
We can also avoid ending up with N+1 queries for nested relations (where N is the total number the type resolver function is called for a given field of the query) by using libraries like graphql-resolve-batch to batch the edges at each node into one resolver invocation (using a special type resolver function that works with arrays of values of a given type from query/mutation result, which are then used in resolving the nested field types, such that it returns a Promise for each value (for the given field) and batches the values by synchronously bucketing under the given field, asynchronously executing the database query with all values for the given field at once during the next tick (assuming the REST API supports querying the db for multiple values at once) c(on NodeJS) and resolving all promises synchronously at once in the callback to the executed query. 
