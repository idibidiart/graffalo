...
Another anti-pattern UI developers often implement is fetching object A, object B and (conditionally) object C from the server then composing object D on the client, possibly with some new fields derived on the client side. This leads to the problem of feature-specific data flow processes (filtering, composing, mapping, reducing, and piping and their imperative equivalent) and business logic are hardcoded in the UI and as requirements change there is a lot of re-work.

With GraphQL sitting on top of performant, entity-mapped CRUD microservices, we get declarative data flow from the type (entity) schema and the ability to isolate business logic in type resolvers.  

This way we can keep all the data flow and business logic out of the UI and the UI state tree can be composed as a pure projection of app state on the server. 
